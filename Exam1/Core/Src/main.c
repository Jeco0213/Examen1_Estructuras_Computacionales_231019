...
// Definimos las variables de nuestro ciclo útil

uint16_t duty = 0; // Variable para almacenar el valor del PWM
uint16_t maxDuty = 0; // Variable para controlar el máximo del ciclo útil
uint8_t rx_buffer[4];
...
int main(void)
{
  ...
  // Iniciamos el contador del Timer 16 en modo Interrupción
  ...
  while (1)
  {
	  ...
	  for (duty = 0; duty < maxDuty; ++duty) {
		  ...
		  // Imprimir el valor del PWM cada 1 segundo
		  HAL_Delay(1000);
		  printf("PWM Value: %d\r\n", duty);
